...
def custom_handler(model_dir: str = "/repository") -> Any:
    ...
    if handler_file.endswith(".py"):  # type: ignore
...
# NOTE: the usage of `pipeline.py` instead of `handler.py` is deprecated, but still included here for backwards
# compatibility making sure that none of those is broken
def custom_pipeline(model_dir: str = "/repository") -> Any:
    model_dir = Path(model_dir)  # type: ignore

    handler_path = model_dir / "pipeline.py"  # type: ignore
    if not handler_path.is_file():
        raise FileNotFoundError(f"`pipeline.py` file not found in the directory `{model_dir}`")

    spec = importlib.util.spec_from_file_location("pipeline.PreTrainedPipeline", handler_path)  # type: ignore
    if spec is None or spec.loader is None:
        raise ImportError(f"Could not load module from `{handler_path}`")
    ...
    if not hasattr(module, "PreTrainedPipeline"):  # type: ignore
        raise AttributeError("The class `PreTrainedPipeline` was not found in `pipeline.py`")

    PreTrainedPipeline = getattr(module, "PreTrainedPipeline")  # type: ignore
    return PreTrainedPipeline(model_dir=str(model_dir))
